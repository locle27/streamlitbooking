...
Version: 3.0.3 (Fixed AttributeError in message templates, improved copy functionality with guidance)
"""
...
st.set_page_config(
    ...
    menu_items={
        ...
        'About': "# Hotel Management System v3.0.3\nSửa lỗi AttributeError và cải thiện sao chép mẫu tin nhắn."
    }
)
    ...
    for line in cleaned_content.splitlines():
        ...
        if current_category:
             ...
             if potential_sub_label_named_match:
                 if potential_sub_label_named_label := potential_sub_label_named_match.group(1).strip():
                     if potential_sub_label_named_label != current_category and potential_sub_label_named_label.isupper() is False:
...
if "add_booking_check_in_val" not in st.session_state:
    st.session_state.add_booking_check_in_val = datetime.date.today()
if "add_booking_check_out_val" not in st.session_state:
    st.session_state.add_booking_check_out_val = datetime.date.today() + timedelta(days=1)
...
if uploaded_file is not None:
    if st.session_state.uploaded_file_name != uploaded_file.name or st.session_state.df is None:
        ...
        if df_from_file is not None and not df_from_file.empty:
            ...
            st.session_state.room_types = sorted(df_from_file['Tên chỗ nghỉ'].unique().tolist()) if 'Tên chỗ nghỉ' in df_from_file.columns and df_from_file['Tên chỗ nghỉ'].nunique() > 0 else []
    ...
    if st.session_state.df is not None and not st.session_state.df.empty:
        st.session_state.room_types = sorted(st.session_state.df['Tên chỗ nghỉ'].unique().tolist()) if 'Tên chỗ nghỉ' in st.session_state.df.columns and st.session_state.df['Tên chỗ nghỉ'].nunique() > 0 else []
...
with tab_dashboard:
    ...
    if df is not None and not df.empty and active_bookings is not None:
        ...
        total_expected_revenue_all_active_dashboard = active_bookings['Tổng thanh toán'].sum() if active_bookings is not None and 'Tổng thanh toán' in active_bookings.columns and not active_bookings.empty else 0
        with col3:
        ...
        if st.button("🔄 Tải lại dữ liệu demo", key="reload_demo_dashboard"):
            st.session_state.df, st.session_state.active_bookings = create_demo_data()
            if st.session_state.df is not None: st.session_state.room_types = sorted(st.session_state.df['Tên chỗ nghỉ'].unique().tolist())
...
with tab_booking_mgmt:
    ...
    if df is not None and not df.empty:
        ...
        with filter_cols_main[0]:
            unique_statuses_mgmt = sorted(df['Tình trạng'].unique().tolist()) if 'Tình trạng' in df.columns else []
            status_filter_manage = st.multiselect("Trạng thái:", options=unique_statuses_mgmt, default=unique_statuses_mgmt, key="status_filter_manage_tab3")
        with filter_cols_main[1]:
            unique_room_types_manage = sorted(room_types)
        ...
        if additional_filter_column_actual and additional_filter_column_actual in df_filtered_for_table.columns:
            if additional_filter_column_actual == "Thành viên Genius":
                unique_genius_values = sorted(df_filtered_for_table["Thành viên Genius"].unique())
                selected_genius_values = st.multiselect(
                    "Lọc theo Thành viên Genius:", options=unique_genius_values, default=unique_genius_values, key="additional_genius_filter_multiselect"
                )
                ...
                unique_currency_values = sorted(df_filtered_for_table["Tiền tệ"].unique())
                selected_currency_values = st.multiselect(
                    "Lọc theo Tiền tệ:", options=unique_currency_values, default=unique_currency_values, key="additional_currency_filter_multiselect"
                )
                    ...
                    with action_buttons_cols[1]:
                        ...
                        if st.button("🗑️", key=delete_single_button_key, help=f"Xóa ĐP {original_booking_id_action}", use_container_width=True):
                            if st.session_state.df is not None and 'Số đặt phòng' in st.session_state.df.columns:
                                ...
                                if not st.session_state.df.empty and 'Tên chỗ nghỉ' in st.session_state.df.columns:
                                    st.session_state.room_types = sorted(st.session_state.df['Tên chỗ nghỉ'].unique().tolist())
                                else:
...
with tab_add_booking:
    ...
    if not room_types: st.warning("Không có thông tin loại phòng. Vui lòng tải file dữ liệu trước.")
    else:
        with st.form(key="add_booking_form_key_v2"):
            st.subheader("Thông tin đặt phòng")
            col_form_add1, col_form_add2 = st.columns(2)
            with col_form_add1:
                guest_name_form = st.text_input("Tên khách*", placeholder="Nhập tên đầy đủ", key="guest_name_key_form_add")
                room_type_form = st.selectbox("Loại phòng*", options=room_types, key="room_type_key_form_add")
                genius_df_source_add = st.session_state.get('df')
                genius_options_add = sorted(genius_df_source_add['Thành viên Genius'].unique().tolist()) if genius_df_source_add is not None and not genius_df_source_add.empty and 'Thành viên Genius' in genius_df_source_add.columns else ["Không", "Có"]
                genius_member_form = st.selectbox("Thành viên Genius", options=genius_options_add, index=0, key="genius_member_key_form_add")
            with col_form_add2:
                check_in_date_form = st.date_input(
                    "Ngày check-in*",
                    value=st.session_state.add_booking_check_in_val, 
                    min_value=datetime.date.today() - timedelta(days=730),
                    max_value=datetime.date.today() + timedelta(days=730),
                    key="add_booking_check_in_val" 
                )
                min_checkout_date = st.session_state.add_booking_check_in_val + timedelta(days=1)
                checkout_val_from_session = st.session_state.get("add_booking_check_out_val", min_checkout_date)
                if checkout_val_from_session < min_checkout_date:
                    actual_default_checkout = min_checkout_date
                    st.session_state.add_booking_check_out_val = actual_default_checkout
                else:
                    actual_default_checkout = checkout_val_from_session
                check_out_date_form = st.date_input(
                    "Ngày check-out*",
                    value=actual_default_checkout,
                    min_value=min_checkout_date,
                    max_value=st.session_state.add_booking_check_in_val + timedelta(days=731), 
                    key="add_booking_check_out_val"
                )
                status_df_source_add = st.session_state.get('df')
                status_options_add = sorted(status_df_source_add['Tình trạng'].unique().tolist()) if status_df_source_add is not None and not status_df_source_add.empty and 'Tình trạng' in status_df_source_add.columns else ["OK", "Đã hủy", "Chờ xử lý"]
                try: default_status_index = status_options_add.index("OK")
                except ValueError: default_status_index = 0
                booking_status_form = st.selectbox("Trạng thái đặt phòng", options=status_options_add, index=default_status_index, key="booking_status_key_form_add")
            
            st.markdown("---"); st.subheader("Thông tin thanh toán")
            col_form_add3, col_form_add4 = st.columns(2)
            with col_form_add3:
                total_payment_form = st.number_input("Tổng thanh toán (VND)*", min_value=0, value=500000, step=50000, format="%d", key="total_payment_key_form_add")
                default_commission = int(total_payment_form * 0.15) if total_payment_form > 0 else 0
                commission_form = st.number_input("Hoa hồng (VND)", min_value=0, value=default_commission, step=10000, format="%d", key="commission_key_form_add")
            with col_form_add4:
                currency_df_source_add = st.session_state.get('df')
                currency_options_add = sorted(currency_df_source_add['Tiền tệ'].unique().tolist()) if currency_df_source_add is not None and not currency_df_source_add.empty and 'Tiền tệ' in currency_df_source_add.columns else ["VND", "USD"]
                try: default_currency_index = currency_options_add.index("VND")
                except ValueError: default_currency_index = 0
                currency_form = st.selectbox("Tiền tệ", options=currency_options_add, index=default_currency_index, key="currency_key_form_add")
                default_booking_id_add = f"MANUAL{datetime.datetime.now().strftime('%y%m%d%H%M%S')}"
                booking_id_form = st.text_input("Mã đặt phòng (tự động nếu trống)", value=default_booking_id_add, key="booking_id_key_form_add")
            
            submitted_form_add = st.form_submit_button("💾 Thêm đặt phòng này", type="primary")
            
            if submitted_form_add:
                errors = [] 
                final_check_in_date = st.session_state.add_booking_check_in_val
                final_check_out_date = st.session_state.add_booking_check_out_val

                if not guest_name_form.strip(): errors.append("Tên khách không được để trống.")
                if final_check_out_date <= final_check_in_date: errors.append("Ngày check-out phải sau ngày check-in.")
                ...
                if st.session_state.df is not None and not st.session_state.df.empty and 'Số đặt phòng' in st.session_state.df.columns and final_booking_id in st.session_state.df['Số đặt phòng'].values:
                    errors.append(f"Mã đặt phòng '{final_booking_id}' đã tồn tại.")

                if not errors and booking_status_form == "OK": 
                    if st.session_state.active_bookings is not None and room_types:
                        current_check_date_form_add = final_check_in_date 
                        while current_check_date_form_add < final_check_out_date: 
                            availability_check_specific_add = get_room_availability(current_check_date_form_add, st.session_state.active_bookings, [room_type_form], ROOM_UNIT_PER_ROOM_TYPE)
                            ...
                            temp_active_bookings_for_add = st.session_state.active_bookings[
                                (st.session_state.active_bookings['Check-in Date'].dt.date <= current_check_date_form_add) & 
                                (st.session_state.active_bookings['Check-out Date'].dt.date > current_check_date_form_add) & 
                                (st.session_state.active_bookings['Tình trạng'] != 'Đã hủy')
                            ]
                            if len(temp_active_bookings_for_add) + 1 > TOTAL_HOTEL_CAPACITY:
                                ...
                                break
                            current_check_date_form_add += datetime.timedelta(days=1)
                
                if errors: 
                    for error_msg in errors: st.error(error_msg)
                else: 
                    default_location = "N/A (Chưa xác định)"
                    if st.session_state.df is not None and not st.session_state.df.empty and 'Tên chỗ nghỉ' in st.session_state.df.columns and 'Vị trí' in st.session_state.df.columns:
                        room_specific_locations_df = st.session_state.df[st.session_state.df['Tên chỗ nghỉ'] == room_type_form]
                        if not room_specific_locations_df.empty:
                            unique_room_locations = room_specific_locations_df['Vị trí'].unique()
                            if len(unique_room_locations) > 0 and pd.notna(unique_room_locations[0]):
                                default_location = unique_room_locations[0]
                    ...
                    if st.session_state.df is None or st.session_state.df.empty: st.session_state.df = new_booking_df_row
                    else: st.session_state.df = pd.concat([st.session_state.df, new_booking_df_row], ignore_index=True)
                    ...
                    if "add_booking_check_in_val" in st.session_state:
                        del st.session_state.add_booking_check_in_val
                    if "add_booking_check_out_val" in st.session_state:
                        del st.session_state.add_booking_check_out_val
                    st.rerun()
    ...
    if not st.session_state.get('message_templates_dict'):
        ...
        for category, labeled_messages in sorted(st.session_state.message_templates_dict.items()):
            with st.expander(f"Chủ đề: {category}", expanded=False):
                ...
                for i, (label, message) in enumerate(labeled_messages):
                    ...
                    col1_msg, col2_msg = st.columns([4,1]) # Chia cột để đặt nút Sao chép
                    ...
                    with col1_msg:
                        ...
                        st.text_area(
                            label=f"_{label}_in_{category}_content_display_", # Nhãn ẩn, chỉ dùng cho Streamlit, thêm _display_ để tránh trùng
                            value=message,
                            height=max(80, len(message.split('\n')) * 20 + 40),
                            key=f"{widget_key_prefix}_text_area_display", # Key mới
                            disabled=True,
                            help="Nội dung tin nhắn. Bạn có thể chọn và sao chép thủ công từ đây."
                        )
                    
                    with col2_msg:
                        st.write("") # Thêm khoảng trống để nút căn chỉnh tốt hơn với text_area
                        st.write("") 
                        if st.button("Sao chép", key=f"{widget_key_prefix}_copy_button", help=f"Nhấn để nhận hướng dẫn sao chép tin nhắn '{label if label != 'DEFAULT' else 'này'}'"):
                            # Hiển thị thông báo toast hướng dẫn người dùng
        ...
        if isinstance(current_raw_template_content, str):
            st.download_button(
                ...
                mime="text/plain",
                key="download_message_templates_button_v2" # Giữ nguyên key nếu không có lý do thay đổi
            )
        else:
...
if st.sidebar.button("🔄 Làm mới dữ liệu & Tải lại từ đầu", key="refresh_data_button_key_final_v2", help="Xóa toàn bộ dữ liệu và bắt đầu lại."):
    keys_to_clear_sidebar = [
        ...
        'add_booking_check_in_val', 'add_booking_check_out_val',
